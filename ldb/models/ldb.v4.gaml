...
global {
  ...
  // simulation control
  string parameter_name <- "../includes/parameters1.v4.json";
...
//  bool   save;  // whether to save the resulting data
  ...
  map<string, map<string,unknown>> parameters <- parameterFile.contents;

  // constants for time
  int                 day_per_month; // nb of days per month
  list<string>        months;        // list of month names
  map<string,map<string,float>> seasons;       // list of season qualities (good, bad, etc.)
...
//  	  crop_residue_price <- int(eco_parameters["prixResidus"]);
	...
	action compute_indicators(bool rewrite) {
	  ...
	  minifarmNb          <- minifarms sum_of (each.milking_nb);
      ...
      complement4minifarm <- minifarms sum_of (each.complement_biomass) / max([1,length(minifarms)]);
	  // collected farms
	  list<Herd> collected <- Herd where (!each.transhumance and each.htype  != "minifarm" and each.collectPoint != nil);
	  list<Herd> milking_collected <- collected where (each.milking_nb != 0);
	  collectedNb          <- collected sum_of (each.milking_nb);
	  milk4collected       <- milking_collected sum_of (each.milk / each.milking_nb) / max([1,length(milking_collected)]);
      ...
      complement4collected <- collected sum_of (each.complement_biomass) / max([1,length(collected)]);
	  // traditional farms
	  ...
	  traditionalNb      <- traditional sum_of (each.milking_nb);
      ...
      complement4others  <- traditional sum_of (each.complement_biomass) / max([1,length(traditional)]);
	  // others
	  ...
	  if (save) {
		save [month,biomass4milk,
			  ...
			  pastoral4minifarm,pastoral4collected,pastoral4others,residue4minifarm,residue4collected,residue4others,complement4minifarm,complement4collected,complement4others
...
species Herd {
	...
	float complement_biomass;  // monthly consumed residue biomass per head
...
//  	bool collectPointaround <- false;
	...
	action herd_produce_milk(int month) {
	  ...
	  float available_biomass <- current_biomass * grass_surface / cowNb  * (cow_ingestion_ratio  / 100);
	  if (milking_nb > 0 and not transhumance) {
	    // herd consumption
	    if (htype = "minifarm") {
	      float prod      <- rnd(min_mf_milk_prod,max_mf_milk_prod); // milk production per head per day
	      float ufl       <- prod / ufl2liter + upkeep_ufl;          // ufl necessary for this production per head per day
//  		write '  ufl needed: '+ufl;
	      // complement and pasture strategy
	      if (is_rainy_season(month)) {
	  		// complement use
	      	complement_biomass <- mf_complementRainSeason * day_per_month; // per head per month
	      	ufl <- ufl - mf_complementRainSeason / cbiomass2ufl;           // per head per day
//  		write '  ufl with complement: '+ufl;
		    // remaining pasture use
		    pasture_biomass <- min([available_biomass, maximum_consumption_metisse * day_per_month - complement_biomass]); // per head per month
		    ufl <- ufl - pasture_biomass / day_per_month / pbiomass2ufl[month]; // per head per day
//  		write '  ufl with pasture: '+ufl;
	      } else {
	      	// only complement
	      	complement_biomass <- mf_complementDrySeason * day_per_month;  // per head per month      	
	      	ufl <- ufl - mf_complementDrySeason / cbiomass2ufl;            // per head per day
//  		write '  ufl with complement: '+ufl;
	      }
	      expense <- expense + (complement_biomass * milking_nb * complement_price);   // cost for complement
	      // residue
	      residue_biomass <- (ufl / rbiomass2ufl) * day_per_month;       // monthly residue biomass per head necessary for this production
	      expense <- expense + (residue_biomass * milking_nb * crop_residue_price);    // cost of crop residue
	      milk    <- prod * day_per_month * milking_nb;                  // total milk production
	    } else {
	      float remaining_biomass <- maximum_consumption_zebu * day_per_month;
	      // complement strategy
	      if (collectPoint != nil and collectPoint.csp) {  // complement available
		    ...
		    expense <- expense + complement_biomass * complement_price * milking_nb;
		    remaining_biomass <- remaining_biomass - complement_biomass;
	      }	      
	      // actually consumed pasture biomass
	      pasture_biomass <- min([available_biomass, remaining_biomass]);
		  remaining_biomass <- remaining_biomass - pasture_biomass;
	      // crop residue biomass from crops
	      if (remaining_biomass > 0) {                      // not enough
	        if (stock_month = 0) {                          // not used yet
	          stock_month <- stock / (12 - month);          // distribute on remaining months
	        }
	        residue_biomass <- min([stock_month / milking_nb, remaining_biomass]); // get the residue quantity per milking cow
	        remaining_biomass <- remaining_biomass - residue_biomass;
	      }
	      // crop residue biomass from csp
	      if (remaining_biomass > 0 and collectPoint != nil and collectPoint.csp) { // further crop residue available
		     residue_biomass <- residue_biomass + remaining_biomass;
		     expense         <- expense + remaining_biomass * crop_residue_price * milking_nb;
	      }
	      // if starving
	      if ((pasture_biomass + residue_biomass + complement_biomass) < starving_limit) {
	      	if (herd_transhumance_decision(month)) {               // transhumance
	        	// the herd goes out of the system for the next turn
	        	transhumance    <- true;
	        } else if (collectPoint != nil and collectPoint.csp) { // complementation
	        	float remaining_complement <- starving_limit - pasture_biomass - residue_biomass - complement_biomass;
	        	expense            <- expense + remaining_complement * complement_price * milking_nb;
	        	complement_biomass <- complement_biomass + remaining_complement;
	        }
	      }
	      // monthly production on all the milking cows
	      milk <- monthly_milk_production(pasture_biomass, residue_biomass, complement_biomass, month) * milking_nb;
	    }
	  } else {
	    milk <- 0.0;
	  }
	...
	bool is_rainy_season(int month) {
		if (month >= 1 and month <= 3) {return true;} else {return false;}
	}
	...
	float monthly_milk_production(float pb,float crb,float cb,int month) {
	  float  ufl <- pb * pbiomass2ufl[month] + crb * rbiomass2ufl + cb * cbiomass2ufl; // UFL from all sources
...
experiment main type: gui {
	parameter 'Fichier paramètre'  var: parameter_name category: 'Simulation' init: "../includes/parameters1.v4.json";
